...
module.exports = (app) => {
    ...
    // Пример маршрута для поиска автомобилей
app.get('/search_cars', async (req, res) => {
    const query = req.query.query; // Получаем запрос из параметров
    try {
        const result = await pool.query('SELECT * FROM cars WHERE vin_car ILIKE $1 OR make_car ILIKE $1 OR model_car ILIKE $1', [`%${query}%`]);
...
<div class="container" style="padding-bottom:100px; color: white;">
    ...
    <form action="/createDeal" method="POST" enctype="application/x-www-form-urlencoded">
        ...
        <div class="form-group">
            ...
            <div>
                <input type="checkbox" id="service_1" name="additional_services_id[]" value="1">
                <label for="service_1">Детское кресло</label>
            </div>
            <div>
                <input type="checkbox" id="service_2" name="additional_services_id[]" value="2">
                <label for="service_2">Доставка автомобиля</label>
            </div>
            <div>
                <input type="checkbox" id="service_3" name="additional_services_id[]" value="3">
                <label for="service_3">Возврат автомобиля</label>
            </div>
        </div>
...
<script>
// Функция для автоматического заполнения полей
async function fillAutoFields() {
    try {
        const response = await fetch('/getCurrentUser'); // Запрос на получение текущего пользователя
        if (!response.ok) throw new Error('Ошибка при получении данных пользователя.');

        const user = await response.json();
        
        document.getElementById('user_id').value = user.user_id; // Заполнение ID пользователя
        document.getElementById('first_name').value = user.first_name; // Заполнение имени
        document.getElementById('surname').value = user.surname; // Заполнение фамилии
        document.getElementById('telephone_number').value = user.telephone_number; // Заполнение номера телефона
    } catch (error) {
        console.error(error.message);
    }
}

// Заполнение полей при загрузке страницы
document.addEventListener('DOMContentLoaded', fillAutoFields);
</script>
...
app.get('/getDeal/:dealId', async (req, res) => {
    ...
    try {
        ...
        console.log(result.rows[0]); // Для отладки: выводим результат запроса в консоль

        res.json(result.rows[0]); // Отправляем данные по сделке в формате JSON
    } catch (error) {
        console.error(error);
        res.status(500).send(`Ошибка при получении данных по сделке: ${error.message}`);
    }
});

app.get('/all_deal', isAdmin || isModer, async (req, res) => {
    try {
        const query = `
            SELECT 
                d.deal_id,
        ...
        res.json(result.rows); // Отправляем все сделки в формате JSON
    } catch (error) {
        console.error(error);
